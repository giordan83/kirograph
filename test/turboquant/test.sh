...
cmd "read src/models/User.ts --mode exports"
$KG read src/models/User.ts --mode exports 2>&1 | sed 's/^/     /'
...
cmd "affected src/services/AuthService.ts"
AFF_OUT=$($KG affected src/services/AuthService.ts 2>&1)
echo "$AFF_OUT" | sed 's/^/     /'
echo ""
...
cmd "path AuthService PaymentService"
PATH_OUT=$($KG path AuthService PaymentService 2>&1)
echo "$PATH_OUT" | sed 's/^/     /'
echo ""
echo "$PATH_OUT" | grep -qi "AuthService\|PaymentService\|Path\|No path\|symbol\|→" \
...
echo "$EXEC_OUT" | sed 's/^/     /'
echo ""
echo "$EXEC_OUT" | grep -qi "OrderService\|AuthService\|models\|services\|\.ts" \
