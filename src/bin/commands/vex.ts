...
export function register(program: Command): void {
  program
    .command('vex [projectPath]')
    .description('Export CycloneDX 1.5 VEX (Vulnerability Exploitability eXchange)')
    .option('--output <file>', 'Write VEX to file instead of stdout')
    .action(async (projectPath: string | undefined, opts: { output?: string }) => {
      ...
      const exporter = new VEXExporter(db, target);
      ...
      if (opts.output) {
        ...
        console.error(`  ${green}✓${reset} VEX written to ${violet}${bold}${outPath}${reset}`);
      } else {
