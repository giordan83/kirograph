...
export function register(program: Command): void {
  program
    .command('ast-rewrite <file> <pattern> <rewrite> [projectPath]')
    .description('Structural rewrite via ast-grep (requires ast-grep on PATH)')
    .action(async (file: string, pattern: string, rewrite: string, projectPath: string | undefined) => {
      const target = path.resolve(projectPath ?? process.cwd());
      await runTool(target, 'kirograph_ast_grep_rewrite', { file, pattern, rewrite });
    });
}
