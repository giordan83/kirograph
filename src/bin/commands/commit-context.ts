...
export function register(program: Command): void {
  program
    .command('commit-context [projectPath]')
    .description('Structured summary of staged changes, ready for a commit message')
    .action(async (projectPath: string | undefined) => {
      const target = path.resolve(projectPath ?? process.cwd());
      await runTool(target, 'kirograph_commit_context', {});
    });
}
